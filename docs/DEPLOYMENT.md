...
已经在跑的机器上换新版本,只有三步。**新功能一律默认关闭**,换完 exe 还要去设置页打开——升级本身不会改变现有行为。
...
产物在 `src\LiveControlPanel\bin\Release\net8.0-windows\win-x64\publish\LiveControlPanel.exe`,约 47 MB,自包含。

**二、替换**(直播机,管理员 PowerShell):

```powershell
Stop-Process -Name LiveControlPanel -Force
Copy-Item <新的>\LiveControlPanel.exe C:\LiveControlPanel\ -Force
Start-Process C:\LiveControlPanel\LiveControlPanel.exe
```

**结束面板不会中断正在进行的直播**(推流是 OBS 在做),但仍然不要在服务期间做。不手动启动的话,下次登录时计划任务会拉起来。

**三、确认面板是提权运行的**:

```powershell
(Get-ScheduledTask -TaskName LiveControlPanel).Principal.RunLevel
# 应当输出 Highest
```

「重置采集卡」这类功能需要管理员权限。按 6.1 节脚本注册的任务本来就带 `-RunLevel Highest`;若不是,按 6.1 删掉重建——否则点按钮会得到「面板需要以管理员身份运行」。

**四、去设置页打开新功能。** 换 exe 只是让功能存在,不会自动启用:

| 新功能 | 在哪开 |
| --- | --- |
| 采集卡恢复(「重置采集卡」按钮) | 设置页「采集卡恢复」,见 **11.4** |
| 画面静止自检(识别 No Signal 提示图) | 设置页 OBS 一段的「检查画面是否静止的来源」,见 **11.4** |
| 双语直播 | 设置页「AI 翻译」,见 **8.7**(还需先做 3.4 与 7.7) |

> 版本没换对的判断方式:设置页里**看不到**上表提到的那些字段,就说明还在跑旧 exe。
...
**再关掉「快速启动」**(管理员 PowerShell 一条命令):

```powershell
powercfg /hibernate off
```

Win10/11 默认开着「快速启动」,它让**关机再开机不走真正的冷启动**——从休眠文件恢复内核会话,驱动不重新初始化。USB 采集卡在这条路上回来时状态经常是坏的:**OBS 显示摄像头无信号,重启 OBS 也没用,只有拔插 USB 或完全重启才能恢复。**

「重启」会真正重初始化驱动所以没事,「关机再开机」不会——所以症状表现为**「特别是刚开机的时候」**。这台机器配了 UPS,断电 → UPS → 关机 → 再开机走的恰好是这条路。

关掉休眠会连带关掉快速启动,并省下一个和内存等大的 `hiberfil.sys`。**这台机器不需要休眠**(它要么开着,要么彻底断电)。

> 想通过控制面板做也可以:控制面板 → 电源选项 → 选择电源按钮的功能 → **更改当前不可用的设置** → 取消勾选「启用快速启动(推荐)」。
...
- **多出一张「AI 翻译」卡片**,一行状态:
  ...
  - 红色「翻译那条直播已经断了」= 第二路推流停了(通常是 obs-multi-rtmp 没启动或密钥错),提示会指向 OBS 的多路推流面板。**这种情况下不会给「重新连接翻译」按钮** —— 译音没有地方可去,重连没有意义,要修的在 OBS 那边
...
8. [ ] 配了采集卡恢复的话:**直播进行中**点一次「重置采集卡」,确认 ① 画面短暂中断后自己回来 ② **YouTube 直播没有中断** ③ 按钮旁出现上次重置时间。这一项要在正式上线前做过一次,否则凌晨没人敢按它
...
| **刚开机时摄像头无信号(重启 OBS 没用,拔插采集卡 USB 才好)** | 头号嫌疑是 Windows **快速启动**——关机再开机不重初始化驱动。按 **4.1** 关掉它。判定测试与完整排查见 **11.4** |
| **画面没了,重启摄像机和重启 OBS 都没用** | 采集卡卡死,只有让 Windows 重新枚举 USB 设备才能清掉。见 **11.4** —— 配置过的话操作页有「重置采集卡」按钮,否则拔插采集卡 USB 或重启电脑 |
| **自检报「画面没有变化」** | 有画面但一秒多内完全没变,通常是摄像机没信号,或采集卡卡死后显示自己的 No Signal 提示图。见 **11.4** |
| **自检报「…在 OBS 里不存在」** | 设置页里配的来源名与 OBS 里的实际名字不一致(常见于改过来源名之后)。用设置页 OBS 一段的「列出 OBS 里的输入名」核对并改正 |
| **保存设置时被拒:「采集卡恢复开着但没有选中设备」** | 设备列表还没加载完就点了保存。等下拉里出现设备、选中采集卡,再保存 |
...
| **页面显示不动/疑似卡死** | 刷新页面;iPad 上锁屏再解锁会强制重连。页面自己也会在 20 秒收不到推送时显示「连接中断」并重连,所以看到那条横幅是正常的自愈,不是故障 |
| **直播卡上出现红色「OBS 正在重连 YouTube」** | OBS 与 YouTube 摄取端断了,**现在观众收不到画面**。检查网络/上行。注意这条横幅下面的时长和码率在重连期间仍会照常走,不代表还在正常直播 |
| **结束直播时提示「面板连不上 OBS,没法确认推流已经停止」** | 面板与 OBS 的 WebSocket 断了。**这时候这场还没真正结束** —— 直接在 OBS 里点「停止推流」,再回到本页点结束 |
...
### 11.4 摄像头无信号 / 采集卡卡死(重启摄像机和重启 OBS 都没用)

**症状:** 画面没了,OBS 显示摄像头无信号。重启摄像机没用,重启 OBS 也没用。最后靠重启电脑、或者拔插采集卡的 USB 线才恢复。**「特别是刚开机的时候」是一个很强的线索**——见下面那个判定测试。

**原因:** 采集卡卡在了一个坏状态里,需要 Windows **重新枚举 USB 设备**才能清掉。重启 OBS 只是关掉一个应用;重启摄像机只动 HDMI 那一端 —— **两者都不会重新枚举 USB 设备**,只有重启电脑或拔插 USB 才会。

#### 先分清坏在哪一段(下次故障时按这个顺序,先别拔线)

采集卡有 **HDMI IN + HDMI OUT(直通)+ USB** 三个口,而**直通是硬件通路**:卡只要有 USB 供电,直通就出图,与驱动状态、与 OBS 完全无关。这给了一个不花钱的决定性诊断。

| 步骤 | 看什么 | 意义 |
| --- | --- | --- |
| **1** | **HDMI OUT 直通**接一台显示器/电视,有没有画面 | **有画面** → 摄像机→HDMI→卡这段是好的,坏在**卡→USB→驱动→OBS**,继续往下看;**也是黑的** → 坏在**摄像机→HDMI→卡**,根本不是采集卡的事,直接跳到「摄像机侧」 |
| **2** | **卡上的状态灯** | 免费告诉你「卡认不认得 HDMI 输入」(常亮/闪烁/不亮的含义看该型号说明) |
| **3** | 摄像机**遥控器能不能转动云台** | **转不了** → 摄像机没完成初始化或供电不足(和无信号同一个根因),查它的电源适配器与插排;**能转** → 遥控器那事与此无关(能关机不能转动多半只是电池弱:关机是一次短码,转动要长按持续发码) |
| **4** | 点面板「状态」卡片的**「重置采集卡」**(若已配好) | **有效** → 以后不用再爬到机器后面拔线;**无效** → 是固件级卡死,需要真正断电,软件禁用/启用不切断供电 |
| **5** | 以上都记下来之后,再拔插 USB 恢复 | |

**第 1 步最关键。** 如果直通也是黑的,那所有关于 USB 和采集卡的排查都是找错方向——拔插 USB「看起来有效」可能只是因为那期间摄像机自己缓过来了。

#### 「特别是刚开机」的话,先做这个判定测试

不用等下次故障,今天就能验证快速启动这个假设:

1. **完全关机**(开始菜单 → 关机)→ 开机 → 看有没有信号
2. 无论结果,**重启**(开始菜单 → 重启)→ 看有没有信号
3. 各做两三遍

| 结果 | 结论 |
| --- | --- |
| 故障**只跟着「关机再开机」**,从不跟着「重启」 | **快速启动确诊。** 按 4.1 节 `powercfg /hibernate off`,然后各再跑两遍确认 |
| 两种**都会**出 | 不是快速启动,往下看根因处置 |
| 两种**都不**出 | 与「开机」的关系没那么直接,等下次自然发生时按上面的观察顺序做 |

事件查看器可能已经把答案记下来了:**事件查看器 → Windows 日志 → 系统**,筛选来源 `USB` / `UsbHub3` / `usbxhci` / `Kernel-PnP`,对着故障那次的开机时间看。

#### 再做根因处置(这几条免费,而且是真正的修复)

**a. 关掉快速启动**——见 4.1 节。对「刚开机高发」这个症状嫌疑最大。

**b. 关掉 USB 选择性暂停(空闲后高发时嫌疑最大)**

「永不睡眠」只管显示器和硬盘,USB 选择性暂停是完全独立的开关,而它挂起设备后相当一部分采集卡醒不回来。这台机器 24 小时开着,凌晨 04:40 之前已经空闲好几个小时 —— 正是最容易发生的时段。**两处都要关,缺一处无效:**
...
```
设备管理器 → 找到采集卡,以及它所在的每一个
  「USB Root Hub」/「通用 USB 集线器」
  → 属性 → 电源管理 → 取消勾选
    「允许计算机关闭此设备以节省电源」
```

集线器那几个**每一个都要单独取消**,Windows 不会批量应用。

**c. 排掉独占进程**

OBS 被强杀过(第 10 节的「面板崩溃演练」就会强杀进程),或者 Windows 相机 / Teams / Zoom / 微信视频 打开过这张卡,设备就会被独占 —— 新 OBS 拿不到,而重启 OBS 无效,因为占着的不是 OBS。任务管理器看有没有残留 `obs64.exe`。

**d. 供电与 USB 口**

采集卡**直插主板后面板的 USB 口**,不走前面板、不走无源集线器,并且**和调音台的 USB 分到不同的控制器上**(两者都吃带宽,采集卡吃得多)。

这类卡是**总线供电**的(没有独立电源口,全靠 USB 那路供电),所以链路里每多一处接触就多一处隐患:线材尽量用**整根成型**的、不要中间加转接头;插**USB 3 口**(蓝色/带 SS 标识)。故障时在设备管理器里看这张卡,**如果 Windows 把它认成 USB 2 而不是 3,基本就是线材或供电问题**。

**e. 发热**

外置采集盒发热明显,热到一定程度会挂死,症状完全一样。让它离开机箱风口、别压在其它设备上。

**f. 驱动 / 厂商软件**

装了采集卡厂商的录制软件(如 RECentral)的话**卸掉,只留驱动** —— 它的后台服务会抢设备。另外去厂商官网查这个型号有没有固件更新,部分型号的更新条目里明确写了修 USB 挂死。

**一个能缩小范围的问题:** 是**刚开机**、**长时间空闲后**、还是**播了一段时间才**出现的?刚开机 → 基本就是 a;空闲后 → 基本就是 b;播了一两小时后 → 偏 e;随机、和时长无关 → 偏 d。

#### 摄像机侧(第 1 步里直通也是黑的,或第 3 步里云台转不了,就看这里)

这一段和采集卡完全无关,处置也完全不同:

- **供电。** 云台电机的功耗比逻辑板高一个量级。供电不足时逻辑板还活着(红外能收、能响应关机),但**电机不动,HDMI 输出也可能是断的或不稳的**。确认用的是原装适配器、线没有压伤、没有和别的设备挤在一个不够格的插排上。
- **上电顺序。** 摄像机和电脑同时上电时,摄像机通常要 10–30 秒才输出 HDMI,采集卡枚举时那一端是空的,而**部分采集卡不会重新握手**。让摄像机**先于电脑**上电(单独开关,或干脆常开)。
- **摄像机菜单。** 关掉**自动关机 / 待机**;把 **HDMI 输出固定成一个格式**(别用「自动」);有「清洁 HDMI 输出」就打开(菜单和参数叠加在画面上也可能导致握手异常)。
- **中间设备。** 若链路里有 HDMI 分配器/延长器,**临时拆掉直连**测一次。
...
1. 勾选**「在操作页显示『重置采集卡』按钮」**
2. 「要重置的设备」下拉里**选采集卡本身**
3. 「重置后要重新打开的 OBS 来源」填摄像机来源名(如 `主摄像机`),重置完面板会让 OBS 重新打开它
4. 点**保存**

> ⚠️ **一定要选采集卡本身。** 选到 USB 集线器会把键盘、调音台一起带下去;选到调音台会在直播中静音。面板会硬拒绝集线器与输入设备这几类,**但它分辨不出调音台和采集卡**(两者都可能是 `Media` 类)—— 这一格选对了才安全。
>
> 下拉里只列**当前接着**的设备。采集卡换过 USB 口后设备编号会变,要回到这里重新选一次并保存。

出问题时操作员怎么做:操作页状态卡片里点**「重置采集卡」**,按钮变红提示后**再点一次**确认(与「结束直播」同一套两步确认)。面板会把设备禁用 3 秒再启用,然后让 OBS 重新打开来源。按钮旁边会显示上次重置的时间 —— 「我是不是已经试过了」是这时候第一个要知道的事。

**这个操作不会中断正在进行的直播**(重启电脑会)。如果画面还没回来,在 OBS 里点那个来源旁边的**眼睛图标**关掉再打开。

需要面板以管理员身份运行 —— 6.1 节的计划任务脚本本来就带 `-RunLevel Highest`。没提权时面板会直接说这件事。

**万一提示「设备已关闭但重新打开失败」:** 这是最坏的情况,卡现在是禁用状态。去**设备管理器**找到提示里写出的那个设备 → 右键 → **启用设备**;不行就重启电脑。面板已经自动重试了三次才这么报,所以不必再点一次按钮。

**处置二 · 让自检真的能发现「没画面」**

原来的 `video` 自检问的是 OBS「这个来源在出画面吗」,而**采集卡自己那张 "No Signal" 提示图在 OBS 看来是完全正常的画面** —— 自检照样绿,观众看到的是一张提示图。

一次性配置:设置页 OBS 一段里,把**摄像机来源名**填进「检查画面是否静止的来源」并保存。自检会给它连拍两帧、间隔一秒多,两帧完全一样就报「画面没有变化」,并直接给出「重置采集卡」按钮。

> **只填摄像机来源。** 放映用的桌面采集显示一张静止的幻灯片是理所应当的,把它填进来就是自制误报 —— 而**误报会让操作员不再看自检**,那比没有这项检查更糟。留空则完全不做这项检查。
>
> 名字填错时自检会直接报「在 OBS 里不存在」而**不会**假装通过 —— 一个说谎的自检比没有自检更糟。填完用同一段里的「列出 OBS 里的输入名」核对一次。
