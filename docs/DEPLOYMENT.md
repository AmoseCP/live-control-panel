...
**一个能缩小范围的问题:** 是**长时间空闲后**出现的,还是**播了一段时间才**出现的?空闲后 → 基本就是 a;播了一两小时后 → 偏 d;随机、和时长无关 → 偏 c。
