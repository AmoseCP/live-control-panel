...
public sealed class DualBroadcastTests
{
    ...
    public async Task Both_broadcasts_reaching_live_is_reported_as_such()
    {
        ...
        // The outcome first: a failed step explains a status that never advanced, and the message
        // names which step it was.
        Assert.True(outcome.Ok, $"step {outcome.FailedStep}: {outcome.Message.En}");
