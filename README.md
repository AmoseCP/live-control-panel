...
355 个测试，全部不接触真实的 YouTube / OBS / Telegram，也不碰真实的 USB 设备。
...
| `CaptureRecoveryTests` | 默认关闭、未选设备不动手、配置的设备不在时不乱猜、危险设备类即使被手改进配置也拒绝、禁用→启用→让 OBS 重开来源、启用失败重试三次、**设备留在禁用状态时绝不报成功**、并发点击被拒而非排队 |
| `FrozenVideoTests` | 逐字节比较（含缺帧/空帧一律不算冻结）、冻结时的提示明确说「重启摄像机和重启 OBS 都没用」、有配置时给出重置按钮否则给出手动办法；一半用例断言**不报警**：默认不检查、不在节目输出上的来源不判、OBS 渲染不出或请求报错时不判 |
