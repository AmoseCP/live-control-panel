...
## 采集卡卡死：一个观察到的故障与它的两个处置

**现场症状：** 画面没了，**重启摄像机没用，重启 OBS 也没用**。最后靠重启电脑、或者拔插 AVerMedia 采集卡才恢复。

**原因：** 采集卡卡在了一个坏状态里，需要 Windows **重新枚举 USB 设备**才能清掉。重启 OBS 只是关掉一个应用；重启摄像机只动 HDMI 那一端 —— **两者都不会重新枚举 USB 设备**。只有重启电脑或拔插 USB 才会。

根因通常是 USB 选择性暂停（`电源选项 → USB 设置`，以及采集卡与其所在每一个 USB 集线器的 `属性 → 电源管理`）；也可能是有进程独占着设备、供电不足、或发热。**这些是先要排除的，面板不能替代它们。** 下面两件事是在根因之外，把这个故障从「凌晨一个人搞不定」变成「点一下」。

### 处置一：面板上的「重置采集卡」

把设备禁用再启用，等价于拔插 USB，但不用爬到机器后面、也不用重启电脑（**重启电脑会中断正在进行的直播**，这个不会）。

默认关闭。管理员在设置页勾开并**选中采集卡本身**，操作页才会出现这个按钮，并且是**两步确认**（与「结束直播」同一套）。

安全性靠三件事，缺一不可：

1. **重置接口不接受任何设备参数。** `POST /api/capture/reset` 只能作用于管理员在 PIN 后面已经选好的那个设备，任何请求都无法指定别的设备
2. **危险的设备类被硬拒绝**，即使配置文件被手改成那样也一样 —— `USB`（集线器与主控）、`HIDClass`、`Keyboard`、`Mouse`、`Net`、`DiskDrive` 等等。重置集线器会把键盘和调音台一起带下去
3. **默认关闭**，与幻灯片控制同一个理由：它伸手到面板之外，动了这台机器上的东西

面板分辨不出调音台和采集卡（两者都可能是 `Media` 类），所以设置页里选对设备是这个功能安全的前提，那一格旁边写了警告。

实现细节里有两处是特意的：

- **启用失败会重试三次。** 「设备关掉了但没能打开」比它要修的故障更糟，绝不能在第一次拒绝时就放弃
- **真的没能打开时，绝不报成功。** 消息直接写出设备名，让人去设备管理器右键启用；`LastResetAt` 也不会被写上
- **禁用抛了异常不等于没禁用。** WMI 的 `Disable` 是在方法已经执行之后才因非零状态码抛出的（比如「需要重启」那类码 —— 那意味着设备其实关掉了）。所以失败分支先回读设备的真实状态：还健康就报「没能关掉」，看起来关掉了就先把它放回去再说话。两个方向的谎都不能说
- **「开着但没选设备」这个状态不允许存下来。** 设备下拉是异步填的（WMI 查询常要一到三秒），而设置是整段替换的 —— 管理员解锁后两秒内改个别的再保存，就会把已配好的采集卡悄悄擦掉，按钮从操作页消失、自检也不再给这个动作，全程没有任何提示。现在前端在下拉还没填好时不上报选择，后端也直接拒绝这个状态
- 用 WMI 的 `Win32_PnPEntity.Disable/Enable`，而不是去调 `powershell.exe -Command Disable-PnpDevice` —— 后者要多花一秒起进程、只能拿回字符串而不是状态码，还要看这台机器的执行策略。`Disable-PnpDevice` 自己也是调 WMI
- 需要提权。面板的计划任务本来就带 `-RunLevel Highest`；没提权时消息会直接说这件事，而不是抛一个访问被拒的错误码

### 处置二：让自检真的能发现「没画面」

原来的 `video` 自检问的是 OBS「这个来源在出画面吗」（`GetSourceActive`）。问题是**采集卡自己那张 "No Signal" 提示图，在 OBS 看来是完全正常的画面** —— 来源是 active 的，自检照样绿，而观众看到的是一张提示图。部署文档里只能用文字警告操作员「开播前扫一眼 OBS 预览」。

现在自检会对指定来源**连拍两帧、间隔一秒多，两帧完全一样就报「画面没有变化」**，并直接给出「重置采集卡」按钮。

判据是**严格逐字节相等**，不是相似度打分，两个原因：

- 帧是 PNG 编码的。改一个像素会让 deflate 输出面目全非，所以「有百分之几的字节不同」毫无意义
- **活的摄像机永远不会产出两帧完全相同的画面** —— 传感器噪声保证了这一点，而且光线越暗噪声越大，不是越小。所以「相等」是一个非常强的信号，这正是一个**不能误报**的自检需要的

它**抓不到**会动的提示图。这个限制是接受的：替代方案是一个会误报的启发式，而**一个学会了无视自检的操作员，比没有自检更糟**。

同样的取舍决定了另外三处「不判」：默认**一个来源都不检查**（要显式配置）、只判**当前在节目输出上**的来源（没被任何场景显示的来源可能被 OBS 停用，看起来和卡住一样）、以及**只填摄像机** —— 放映用的桌面采集显示一张静止的幻灯片是理所应当的，把它填进来就是自制误报。

但「不判」不等于「装作通过」。**名字填错时自检直接报「在 OBS 里不存在」**：跳过一个拼错的名字会让这一项变绿，而那恰好是这个功能存在的理由 —— 观众看着提示图，自检说画面正常。名字是否存在按 OBS 自己的输入列表核（与声音那一项同一套做法）；列表本身拿不到时**不冤枉任何名字**，只是不做名字校验、照样比较帧。

`FrozenVideoTests` 里一半的用例断言的是**不报警**。
## 双语直播：同一画面，两条语音
...
**API Key 不会进日志。** 它在端点的 query string 里，而日志是明文、留 31 天、七个人共用一台机器。连接失败时只记异常类型和面板自己的文案，不记原始异常 —— .NET 的 WebSocket 失败一般只带主机名，所以泄露是「不太可能」而非「一定」，但这个项目本来就为 Telegram token 做过同样的防护（`Program.cs` 里那个 HttpClient 日志级别覆盖），保持一致只花一行。
...
502 个测试，全部不接触真实的 YouTube / OBS / Telegram，也不碰真实的 USB 设备。
...
| `CaptureRecoveryTests` | 默认关闭、未选设备不动手、配置的设备不在时不乱猜、危险设备类即使被手改进配置也拒绝、禁用→启用→让 OBS 重开来源、启用失败重试三次、**设备留在禁用状态时绝不报成功**、**禁用抛异常但其实生效时按设备真实状态报告并把它放回去**、并发点击被拒而非排队 |
| `FrozenVideoTests` | 逐字节比较（含缺帧/空帧一律不算冻结）、冻结时的提示明确说「重启摄像机和重启 OBS 都没用」、有配置时给出重置按钮否则给出手动办法、**来源名填错时报错而不是假装通过**；一半用例断言**不报警**：默认不检查、不在节目输出上的来源不判、OBS 渲染不出或请求报错时不判、输入列表拿不到时不冤枉每个名字 |
...
| POST | `/api/capture/reset` | 禁用再启用采集卡，让 Windows 重新枚举它。**不接受任何参数** —— 只作用于设置页里已选定的设备 |
...
| GET | `/api/diag/usb-devices` | 列出当前接着的、可作为重置目标的设备（需 PIN；集线器与输入设备已被过滤掉） |
...
11. 可选，但这台机器上很值得：设置页「采集卡恢复」勾开并**选中采集卡本身**，再把摄像机来源填进「检查画面是否静止的来源」。
    前者让操作员一个人就能从采集卡卡死里恢复，后者让自检真的能发现「有画面但画面是提示图」。
    动手之前先把 USB 选择性暂停关掉（`电源选项 → USB 设置`，以及采集卡与其所在每一个 USB 集线器的 `属性 → 电源管理`）—— 那才是根因
