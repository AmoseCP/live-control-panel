...
public sealed class Orchestrator
{
    ...
    private sealed record CreatedBroadcast(
        string Id, string Title, string WatchUrl, bool Adopted, string? LifeCycleStatus = null);
        ...
        string title, string description, ServiceTemplate? template, CancellationToken ct)
    {
        try
        {
            ...
            var leftover = (await _youtube.ListUnfinishedBroadcastsAsync(ct).ConfigureAwait(false))
                .FirstOrDefault(b => b.Title == title && IsFromToday(b));

            if (leftover is not null)
            {
                _log.LogInformation("Adopted existing broadcast {Id} \"{Title}\" instead of creating a duplicate",
                    leftover.Id, leftover.Title);
                return new CreatedBroadcast(
                    leftover.Id, leftover.Title, leftover.WatchUrl, Adopted: true, leftover.LifeCycleStatus);
            }
        }
        catch (Exception ex)
        {
    ...
    private static BroadcastState NewBroadcastState(CreatedBroadcast info, DateTime now) => new()
    {
        ...
        // broadcast that is already live. Recording it as "created" sent step 2 on to bind a live
        // broadcast, which YouTube rejects — and every retry of that step failed the same way, with
        // no path forward that did not end the service.
        Status = AdoptedStatus(info.LifeCycleStatus),
        Title = info.Title,
        CreatedOn = now,
    };
    ...
    private async Task<StepMessage> AwaitTranslatedLiveAsync(CancellationToken ct)
    {
        ...
        while (DateTime.UtcNow < deadline && !ct.IsCancellationRequested)
        {
            ...
            // Already finished on YouTube's side: it is not going to come up, so stop waiting and
            // let the warning below name the OBS target to check.
